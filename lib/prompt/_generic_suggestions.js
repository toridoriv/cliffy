...
var _GenericSuggestions_instances, _GenericSuggestions_hasReadPermissions, _GenericSuggestions_isFileModeEnabled, _GenericSuggestions_completeValue;
import * as dntShim from "../_dnt.shims.js";
import { GenericInput, } from "./_generic_input.js";
import { bold, brightBlue, dim, dirname, join, normalize, stripColor, underline, } from "./deps.js";
...
export class GenericSuggestions extends GenericInput {
    ...
    async render() {
        if (this.settings.files && __classPrivateFieldGet(this, _GenericSuggestions_hasReadPermissions, "f") === undefined) {
            const status = await dntShim.Deno.permissions.request({ name: "read" });
            // disable path completion if read permissions are denied.
            __classPrivateFieldSet(this, _GenericSuggestions_hasReadPermissions, status.state === "granted", "f");
        }
        await this.match();
        return super.render();
    }
    ...
    async getFileSuggestions(input) {
        ...
        const path = await dntShim.Deno.stat(input)
...
_GenericSuggestions_hasReadPermissions = new WeakMap(), _GenericSuggestions_instances = new WeakSet(), _GenericSuggestions_isFileModeEnabled = function _GenericSuggestions_isFileModeEnabled() {
...
async function listDir(path, mode) {
    const fileNames = [];
    for await (const file of dntShim.Deno.readDir(path || ".")) {
